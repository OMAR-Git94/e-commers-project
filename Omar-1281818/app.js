...
const port = 7039;
app.use(express.static(__dirname + "/furnitureroot"));
app.use(
  session({
    secret: "kit2kat",
    resave: false,
...
var carts = [];
var orders = [];
var uploadfolder = path.join(__dirname, "furnitureroot", "images");
/////////////////////////////
connect();
getCategories();
getProducts();
...
app.post("/save-product", async (req, res) => {
  ...
  const [fields, files] = await form.parse(req);
  //console.log('fields', fields.name, fields.featured);
  //console.log('files pic', files.picture[0].newFilename)
  let data = {
    name: fields.name[0],
